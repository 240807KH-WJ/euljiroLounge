...
if (postitList) {
	...
	if (closeDetailModal) {
		...
		detailLike.addEventListener("click", async (e) => {
			...
			currentPostitLikeCount.innerText =
				likeCheck == 0 ? currentLikeCountText + 1 : currentLikeCountText - 1;
		...
		deleteBtn.addEventListener("click", async () => {
			...
			const postNo = deleteBtn.dataset.postNo;

			try {
				const response = await fetch(`/post/${postCode}/${postNo}`, {
					method: "DELETE",
				});
