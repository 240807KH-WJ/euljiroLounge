package com.lounge.euljiro.common.interceptor;
 ...
 * 
 * */
public class PostTypeInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		// application scope 에 "postTypeList" 가 없을 경우
		if (application.getAttribute("postTypeList") == null) {

			// boardTypeList 조회 서비스 호출
			List<Map<String, Object>> postTypeList = service.selectPostTypeList();

			// 조회 결과를 application scope 에 추가
			application.setAttribute("postTypeList", postTypeList);
	...
	// 후처리 : 요청이 처리된 후, 뷰가 렌더링되기 전에 실행되는 메서드 (==> 응답을 가지고 Dispatcher Servlet에게 돌려보내기
	// 전)
	@Override
